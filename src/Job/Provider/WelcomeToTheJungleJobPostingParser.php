...
final class WelcomeToTheJungleJobPostingParser
{
    ...
    public function extractOfferUrls(string $content, int $limit): array
    {
        ...
        // 1. Links matching /companies/.../jobs/...
        foreach ($crawler->filterXPath('//a[contains(@href, "/companies/") and contains(@href, "/jobs/")]') as $link) {
            if (!$link instanceof \DOMElement) {
                continue;
            }

            $href = $link->getAttribute('href');
            if (preg_match('~/companies/([^/]+)/jobs/([^"?#]+)~', $href, $matches) === 1) {
                $urls[] = sprintf('https://www.welcometothejungle.com/fr/companies/%s/jobs/%s', $matches[1], $matches[2]);
        ...
        // 2. Fallback to regex on JSON / __NEXT_DATA__
        if (count($urls) < $limit && preg_match_all('~/companies/([^/"]+)/jobs/([^"?#"]+)~', $content, $matches)) {
            foreach ($matches[0] as $match) {
                $urls[] = str_starts_with($match, 'http') ? $match : 'https://www.welcometothejungle.com'.$match;
                $urls = array_values(array_unique($urls));
                if (count($urls) >= $limit) {
                    break;
                }
    ...
    {
        if (preg_match('~/companies/([^/]+)/jobs/([^"?#]+)~', $url, $matches) !== 1) {
            throw new \RuntimeException(JobMessage::INVALID_URL);
        }
        $externalId = sprintf('%s_%s', $matches[1], $matches[2]);
    ...
    private function buildOfferFromHtml(string $html, string $externalId, string $url): NormalizedJobOffer
    {
        ...
        $titleNode = $crawler->filter('h1, [data-testid="job-header-title"]');
        $title = $titleNode->count() > 0 ? trim($titleNode->first()->text()) : null;
        if ($title === null || $title === '') {
            throw new \RuntimeException(JobMessage::JOB_POSTING_NOT_FOUND);
        }

        $companyNode = $crawler->filter('h2, [data-testid="job-header-company-name"]');
        $company = $companyNode->count() > 0 ? trim($companyNode->first()->text()) : null;

        $locationNode = $crawler->filter('[data-testid="job-header-location"]');
        $location = $locationNode->count() > 0 ? trim($locationNode->first()->text()) : null;

        $descriptionNode = $crawler->filter('[data-testid="job-section-description"], main');
        ...
        return new NormalizedJobOffer(
            ...
            publishedAt: null,
            validThrough: null,
            rawPayload: ['source' => 'wttj_html_fallback'],
        );
    }
